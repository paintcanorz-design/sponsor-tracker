...
- 程式內建版本號見 **`src/version.py`** 的 `APP_VERSION`（目前版本為 `1.13`）。
- **GitHub**：推送 **`v*`** 標籤（例如 `v1.13`）會觸發 Actions 建置並上傳 **`SponsorTracker-Windows-v{version}.zip`**。一鍵更新需要 Release 上的**自訂 zip**（內含 `贊助額追蹤.exe` 與 `_internal`），請勿僅依賴「Source code」自動壓縮檔。
...
- Bump **`APP_VERSION`** in `src/version.py` (current release line is **`1.13`**).
- Push a git tag like **`v1.13`**. The **Release** workflow builds PyInstaller output and uploads **`SponsorTracker-Windows-v1.13.zip`**.
...
- バージョンは **`src/version.py`** の `APP_VERSION`（現在は **`1.13`**）。
- **`v1.13`** のような **タグを push** すると GitHub Actions がビルドし、**`SponsorTracker-Windows-v1.13.zip`** を Release に添付します。
