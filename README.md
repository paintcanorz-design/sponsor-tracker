...
- 程式內建版本號見 **`src/version.py`** 的 `APP_VERSION`（目前版本為 `1.16.1`）。
- **GitHub**：推送 **`v*`** 標籤（例如 `v1.16.1`）會觸發 Actions 建置並上傳 **`SponsorTracker-Windows-v{version}.zip`**。一鍵更新需要 Release 上的**自訂 zip**（內含 `贊助額追蹤.exe`、`_internal`、`開啟exe前請開啟此檔案安裝所需環境.bat`、`使用說明.txt`），請勿僅依賴「Source code」自動壓縮檔。
...
- Bump **`APP_VERSION`** in `src/version.py` (current release line is **`1.16.1`**).
- Push a git tag like **`v1.16.1`**. The **Release** workflow builds PyInstaller output and uploads **`SponsorTracker-Windows-v1.16.1.zip`**.
...
- バージョンは **`src/version.py`** の `APP_VERSION`（現在は **`1.16.1`**）。
- **`v1.16.1`** のような **タグを push** すると GitHub Actions がビルドし、**`SponsorTracker-Windows-v1.16.1.zip`** を Release に添付します。
